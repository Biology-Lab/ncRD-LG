...
def links2subgraphs(Arow, 
    ...
    if not parallel:
        ...
        with open('..data/subgraph.pkl', 'wb') as f:
            pickle.dump(g_list, f)

        with open('../data/Arow.pkl', 'wb') as f:
            pickle.dump(Arow, f)
...
#construct traing and testing set for leave-one-disease-out cross-validation
def con_array_disease(adj):
    num_rows, num_cols = adj.shape
    for col in range(num_cols):
        arr = adj.copy()
        ones_indices = np.where(arr[:, col] == 1)[0]
        zeros_indices = np.where(arr[:, col] == 0)[0]
        
        if col == 0:
          arr[ones_indices, col] = -1
          arr[zeros_indices, col] = -10
        else:
          arr[ones_indices, col] = -1
          selected_zeros_10 = np.random.choice(zeros_indices, len(ones_indices), replace=False)
          arr[selected_zeros_10, col] = -10
        total_ones_in_other_cols = 0
        other_cols = [i for i in range(num_cols) if i != col]
        for other_col in other_cols:
            total_ones_in_other_cols += np.sum(arr[:, other_col] == 1)
        zeros_row_indices,zeros_col_indices = np.where(arr == 0)
        all_other_zeros_indices = zeros_row_indices * arr.shape[1] + zeros_col_indices
        if len(all_other_zeros_indices) >= total_ones_in_other_cols:
            selected_zeros_20 = np.random.choice(all_other_zeros_indices, total_ones_in_other_cols, replace=False)
            arr[selected_zeros_20 // num_cols,selected_zeros_20 % num_cols] = -20
        np.save(f'../data/random_disease/PD_ben_ind_disease_{col}.npy', arr)

#construct indepedent test set
def modify_array_col_balance(arr):
    ...
    for col in range(num_cols):
        ...
        if num_ones < 5:
            ones_split = np.array_split(ones_indices, num_ones)
        else:
            ones_split = np.array_split(ones_indices, 5)
        
        selected_ones = ones_split[np.random.randint(len(ones_split))]
        arr[selected_ones, col] = -1
        zeros_indices = np.where(arr[:, col] == 0)[0]
        len_selected_zeros_20 = len(ones_indices)-len(selected_ones)
        if (col == 0) and (len(zeros_indices) < len(ones_indices)):
            np.random.shuffle(zeros_indices)
            zeros_split = np.array_split(zeros_indices, 5)
            selected_zeros_10 = zeros_split[np.random.randint(len(zeros_split))]
            selected_zeros_20 = np.setdiff1d(zeros_indices, selected_zeros_10)
            arr[selected_zeros_20, col] = -20
            arr[selected_zeros_10, col] = -10
        if (col <=3) and (len(zeros_indices) >= len(ones_indices)) :
            np.random.shuffle(zeros_indices)
            zeros_indices = zeros_indices[0:(len(ones_indices) + 1193)]
            zeros_split = np.array_split(zeros_indices, 5)
            selected_zeros_10 = zeros_split[np.random.randint(len(zeros_split))]
            selected_zeros_20 = np.setdiff1d(zeros_indices, selected_zeros_10)
            arr[selected_zeros_20, col] = -20
            arr[selected_zeros_10, col] = -10
        if (col > 3) and (len(zeros_indices) >= len(ones_indices)) :
            selected_zeros_20 = np.random.choice(zeros_indices, len_selected_zeros_20, replace=False)
            arr[selected_zeros_20, col] = -20
            remaining_zeros_indices = np.setdiff1d(zeros_indices, selected_zeros_20)
            if len(remaining_zeros_indices) >= len(selected_ones):
                selected_zeros_10 = np.random.choice(remaining_zeros_indices, len(selected_ones), replace=False)
                arr[selected_zeros_10, col] = -10
    return arr